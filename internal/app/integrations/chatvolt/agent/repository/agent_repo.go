...
func (r *agentRepo) SalvarConfiguracao(ctx context.Context, integracaoID int64, config map[string]interface{}) error {
	...
	query := `
		INSERT INTO integracoes_configuracoes (integracao_id, configuracoes)
		VALUES ($1, $2)
		ON CONFLICT (integracao_id)
		DO UPDATE SET configuracoes = EXCLUDED.configuracoes, atualizado_em = CURRENT_TIMESTAMP
	`

	_, err = r.db.Exec(ctx, query, integracaoID, jsonData)
	return err
}
